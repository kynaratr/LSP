...
@error('daftar_paket') is-invalid @enderror" placeholder="Cari Daftar Paket" id="daftar_paket" name="daftar_paket" value="{{old('daftar_paket')}}" arialabel="daftar_paket" aria-describedby="cari1" readonly>
...
@error('tgl_reservasi_wisata') is-invalid @enderror" id="tgl_reservasi_wisata" placeholder="Tanggal Pemesanan" name="tgl_reservasi_wisata" value="{{old('tgl_reservasi_wisata')}}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control @error('jumlah_peserta') is-invalid @enderror" id="jumlah_peserta" placeholder="Jumlah Peserta" max='100' name="jumlah_peserta" value="{{old('jumlah_peserta')}}" onkeyup="hargadisk()">
         ...
         <div class="modal fade" id="staticBackdrop1" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable p-5">
                <div class="modal-content">
                    ...
                    <tbody>
                        @foreach($daftarpaket as $key => $a)
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$a->nama_paket}}</td>
                            <td>{{$a->paketwisata2->diskon}}</td>
                            <td>{{$a->jumlah_peserta}}</td>
                            <td>{{$a->harga_paket}}</td>
                                    <td>
                                        <button type="button" class="btn btn-primary
btn-xs" onclick="pilih1('{{$a->id}}', '{{$a->nama_paket}}', '{{$a->jumlah_peserta}}', '{{$a->harga_paket}}', '{{$a->paketwisata2->diskon}}'); hargadisk(); " data-bs-dismiss="modal">
                                            Pilih
                                        </button>
                                    </td>
                                </tr>
        ...
        <script>
            ...
            function pilih1(id_daftar_paket, daftar_paket, harga_paket, jumlah_peserta, diskon) {
                document.getElementById('id_daftar_paket').value = id_daftar_paket
                document.getElementById('daftar_paket').value = daftar_paket 
                document.getElementById('harga').value = jumlah_peserta
                document.getElementById('jumlah_peserta').value = harga_paket
